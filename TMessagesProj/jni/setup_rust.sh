#!/usr/bin/env bash
# Install the pinned Rust toolchain used by jni/prebuild/build_tlottie.sh.
#
...
# F-Droid buildserver. None of them ship one, and none of them agree on where
# one would live, so both homes are fixed literals here and the toolchain
# version is pinned by build.gradle: a different rustc emits different code for
# identical sources. This script is idempotent: it exits immediately once the
# pinned toolchain and all four Android targets are present.
#
