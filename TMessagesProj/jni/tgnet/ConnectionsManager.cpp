...
// Called from the bindFailed path. Returns true if the caller should skip
// cleanUp/logout (we'll retry the handshake — possibly at the same TTL,
// possibly at the next ladder step). Returns false only when we've truly
// exhausted the ladder — caller falls through to the normal cleanUp/logout
// flow as a safety net. On exhaustion we signal the Java side via delegate
// so SharedConfig surfaces the per-account "exhausted" footer.
//
