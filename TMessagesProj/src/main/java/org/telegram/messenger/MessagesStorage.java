...
*/
            ...
            for (int a = 0, N = dialogFilters.size(); a < N + 2; a++) {
                ...
                if (filter != null) {
                    ...
                    if (BuildVars.LOGS_ENABLED) {
                        FileLog.d("filter " + filter.id + " " + filter.name + " flags = " + filter.flags + " unread count = " + filter.pendingUnreadCount);
                    }
                    if (apply) {
        ...
        for (int b = 0; b < 2; b++) {
            ...
            for (int a = 0; a < array.size(); a++) {
                ...
                if (read) {
                    ...
                    // batch of deleted messages), and subtracting it again each time walked the
                    // folder badges down to zero, where they stay until the next full recount.
...
//                }
                ...
                // Mercurygram: new counters of the dialogs whose value changed, so the folder
                // badges can be adjusted incrementally below instead of recomputed from scratch.
                LongSparseIntArray newUnreadCounts = new LongSparseIntArray();
                LongSparseIntArray newMentionsCounts = new LongSparseIntArray();
                for (int a = 0; a < dialogsToUpdate.size(); a++) {
                    long did = dialogsToUpdate.keyAt(a);
                    ...
                    // Mercurygram: a forum reaches a folder badge through its topics, never
                    // through dialogs.unread_count (calcUnreadCounters, putMessagesInternal do
                    // the same), and resetForumBadgeIfNeed below already drops the badge when
                    // the last topic is read. Counting the dialog-level number here too would
                    // subtract the same forum twice on every deletion.
                    if (!isForum(did, FORUM_TYPE_CHAT | FORUM_TYPE_BOT | FORUM_TYPE_DIRECT)) {
                        if (new_unread_count != old_unread_count) {
                            newUnreadCounts.put(did, new_unread_count);
                        }
                        if (new_mentions_count != old_mentions_count) {
                            newMentionsCounts.put(did, new_mentions_count);
                        }
                    }
                ...
                // Mercurygram: resetAllUnreadCounters() reloaded every dialog with unread or
                // flags (hundreds of chats deserialised) on every batch of deleted incoming
                // messages; with server-side auto-delete that ran several times per second and
                // saturated the storage queue. Folder badges count dialogs, so only a counter
                // that dropped to zero can change one: the incremental path used for reads.
                if (newUnreadCounts.size() != 0 || newMentionsCounts.size() != 0) {
                    updateFiltersReadCounter(newUnreadCounts, newMentionsCounts, true);
                    AndroidUtilities.runOnUIThread(() -> getNotificationCenter().postNotificationName(NotificationCenter.updateInterfaces, MessagesController.UPDATE_MASK_READ_DIALOG_MESSAGE));
                }
                // Mercurygram: the adjustment above is an approximation, and a badge that reads
                // too low is clamped at zero and can only come back when a chat goes from read
                // to unread, so on an account where deletions never stop the badges slide to
                // zero and stay there. Recompute them for real, at most once every 30 seconds.
                // A throttle, not a debounce: a runnable cancelled and reposted on every batch
                // would never get to run here.
                if (dialogsToUpdate.size() != 0 && !pendingUnreadCountersResync) {
