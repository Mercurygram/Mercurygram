        ...
        if (state == it.belloworld.mercurygram.tor.MgTorClient.State.PLUGIN_SIGNATURE_MISMATCH) {
            ...
            b.setPositiveButton(LocaleController.getString(R.string.MercurygramTorInstallPlugin),
                    (d, which) -> {
                        ...
                        startInAppPluginInstall(context);
                    });
        }
        showDialog(b.create());
    }

    private void startInAppPluginInstall(Context context) {
        ...
        // Delay 320ms so a fast 404 (e.g. asset missing for the current tag)
        // doesn't flash a spinner before the error toast.
        progress.showDelayed(320);
        MgUpdateChecker.downloadPlugin(new MgUpdateChecker.ProgressCallback() {
            ...
            @Override public void onComplete(java.io.File apkFile) {
                ...
                if (activity != null) MgUpdateChecker.installUpdate(activity, apkFile);
            }
