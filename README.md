...
- Mercurygram folders: a "Mercurygram folder" switch when creating a folder (Settings → Folders → Create new folder) keeps the folder off Telegram's folder API, so it counts against neither the folder limit nor the chats-per-folder limit; once the server limit is hit, "Create new folder" goes straight to one. The switch can also be flipped later on any folder that is not shared: the folder then moves to Mercurygram (deleted from Telegram) or back to Telegram (created there, within its limits), keeping its chats and pins. Mercurygram folders show a "Mercurygram" tag in the folder list. They are synced between your Mercurygram apps through one JSON document in Saved Messages (`mercurygram-folders.json`, edited in place, last writer wins), so Telegram learns nothing it did not already know and no third-party service is involved; the format is documented in [MercurygramFolders.md](MercurygramFolders.md) for the Desktop client and bots
- Pin as many chats as a folder takes (100) in "All Chats", instead of the 5 Telegram allows without Premium. The order is kept on the device and written back over every server refresh, so the pins the server refuses stay put; Telegram still gets the first few as ordinary pins, which is what other clients show. Pins made elsewhere are picked up and moved to the top, and an unpin made on another device while this one was offline for long leaves the chat pinned here until it is unpinned again
...
- Log in by scanning a QR code with a device where the account is already logged in, like Telegram Desktop: tap "Log in by QR code" under the phone number field, then scan the code from the other phone (*Settings → Devices*). No SMS or login code is needed; two-step verification still asks for the password
...
- Fix a custom notification sound on a contact or a chat being reset to Default by opening its *Sound* screen. When the stored tone could not be matched at that moment (a media rescan reassigns the id of a system tone, so the stored one no longer exists) the screen treated its own fallback as a user choice, saved Default over the stored tone and pushed that to the server, which is why a sound often only stuck after several attempts
...
- Fix background live location sharing on devices without Play Services: GPS was switched off before a new fix arrived, so the same stale position was re-sent forever, and a share did not come back after a process kill or a reboot. When location is only allowed while the app is in use, a share restored at boot waits for the app to be opened instead of running blind
- Fix a live location shared "until stopped" being treated as already expired in the chat banner and on the live location map, and a failed live location load that blocked every later load for that chat
...
- **Privacy-respecting translation engine** — *Settings → Mercurygram → Privacy → Translation → Engine* picks how received messages (manual translate, per-chat auto-translate, and the in-chat translate bar) are translated, instead of always hitting Telegram's cloud:
  ...
  - *Alternative HTTP* — routes the text through a public [Mozhi](https://codeberg.org/aryak/mozhi) privacy proxy that aggregates several engines (DuckDuckGo, Google, Yandex); pick the engine and instance. Keeps the query off Telegram's servers.
...
- **Strip tracking parameters**: opt-in per-account toggle at *Settings → Mercurygram → Privacy*. Removes click-tracking query parameters from links you open, from links you paste into a message field, from links you copy out of the app, and from links shared into Mercurygram from another app, so opening a link doesn't report back to the campaign that produced it and forwarding one doesn't carry the sender's identifier to everyone else. Two rule sets: ad-network and campaign parameters that mean nothing anywhere (`utm_*`, `pk_*`, `mtm_*`, `fbclid`, `gclid`, `gbraid`, `msclkid`, `yclid`, `ttclid`, `igsh`, `igshid`, `igsi`, `mc_cid`, `mkt_tok`, `irclickid`, `erid` and about seventy more), plus site-specific ones that are only tracking on their own site and are left alone everywhere else (`si` on YouTube and Spotify, `s`/`t` on X, the `share_*` family on TikTok, `stkn` on Instagram, `pd_rd_*`/`ref` on Amazon, `aff_*` on AliExpress, `ved`/`ei` on Google Search, and a dozen other sites). Parameters that carry real meaning are deliberately kept — Instagram's `img_index`, Amazon's `th` and `psc`. The rest of the address is left byte-identical, and links with nothing to strip are passed through untouched. A copy that had something stripped lands on the clipboard as plain text, since the styled flavour of a clip carries every link a second time inside its HTML and would paste a still-tracked hyperlink. Clipboard content produced by other apps is out of reach: Android offers no hook on another app's copy.
...
- [Rust](https://github.com/Mercurygram/aesgcm-proxy) (own repository), the production gateway, also published as the container image `ghcr.io/mercurygram/aesgcm-proxy`
...
**Prerequisites:** Android SDK with the NDK version pinned by `ndkVersion` in `TMessagesProj/build.gradle`, JDK 17, `git`, and the native toolchain the media libraries need: [Ninja](https://ninja-build.org/), [Meson](https://mesonbuild.com/) (dav1d), `nasm` (x86 assembly in libvpx, ffmpeg and openh264), `make`, `cmake`, `pkg-config`, `gperf` and `python3` (dav1d and TDLib). `curl` is needed too: the build installs the pinned Rust toolchain that `jni/prebuild/build_tlottie.sh` uses (see `TMessagesProj/jni/setup_rust.sh`), because upstream ships the tlottie and openh264 archives prebuilt and Mercurygram builds them from source instead. The same goes for the call core wasm module: the build downloads the pinned [wasi-sdk](https://github.com/WebAssembly/wasi-sdk) (see `TMessagesProj/jni/setup_wasi_sdk.sh`) and compiles it with `TMessagesProj/jni/build_wasm_core.sh`.
...
Native libraries (BoringSSL, openh264, opus, libvpx, dav1d, FFmpeg, tlottie, TDLib) are built from source automatically on the first build and cached for subsequent runs.
