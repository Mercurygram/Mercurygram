...
- **FOSS / offline translation via external app** — opt-in at *Settings → Mercurygram → Privacy → Translation → Engine*. Picks "Offline (external app)" to delegate every message translation to [Offline Translator](https://f-droid.org/packages/dev.davidv.translator/) (GPL-3.0, Bergamot offline models) over an AIDL bound service. Background dispatch — the translator UI never opens — so both manual translate and per-chat auto-translate work, and the message text never leaves your device. Auto-fallback (default on) routes AIDL failures (no model for the language pair, language not detected, service unreachable) to the existing non-MTProto HTTP path; Telegram cloud is never chosen as the fallback. Bold / italic / code formatting can be lost on the offline path because Bergamot returns plain text.
