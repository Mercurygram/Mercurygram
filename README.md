...
Notifications → Disable UnifiedPush*), nothing pushes to the app any more, so
...
**Prerequisites:** Android SDK with the NDK version pinned by `ndkVersion` in `TMessagesProj/build.gradle`, JDK 17, `git`, and the native toolchain the media libraries need: [Ninja](https://ninja-build.org/), [Meson](https://mesonbuild.com/) (dav1d), `nasm` (x86 assembly in libvpx and ffmpeg), `cmake` and `pkg-config`.
