...
- Mercurygram folders: a "Mercurygram folder" switch when creating a folder (Settings → Folders → Create new folder) keeps the folder on this device only, never sent to Telegram, so it counts against neither the folder limit nor the chats-per-folder limit; once the server limit is hit, "Create new folder" goes straight to one. Mercurygram folders show a "Mercurygram" tag in the folder list
