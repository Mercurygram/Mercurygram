 ...
 * for the global engine/model/VAD switches, UserConfig.transcribeLang for the
...
class MgWhisperConfigTest {
    ...
    fun setUp() {
        ...
        savedLang = UserConfig.getInstance(0).transcribeLang
    ...
    fun tearDown() {
        ...
        UserConfig.getInstance(0).transcribeLang = savedLang
    ...
    fun langSelectionPersists() {
        ...
        uc.transcribeLang = "it"
        uc.saveConfig(false)
        assertEquals("it", uc.transcribeLang)
        assertEquals("it", accPrefs.getString("transcribeLang", null))
        uc.transcribeLang = "auto"
        uc.saveConfig(false)
        assertEquals("auto", uc.transcribeLang)
