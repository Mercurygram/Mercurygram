    ...
    private void onUpdateClicked(int currentAccount) {
        ...
        MgUpdateChecker.downloadUpdate(info, new MgUpdateChecker.ProgressCallback() {
            ...
            public void onComplete(File apkFile) {
                ...
                // Fire the system installer immediately (mirrors MgUpdateAlertDialog) so the
                // upgrade proceeds over any foreground fragment — the strip is hidden when the
                // user navigated away during the download. The "Install Now" label above stays
                // as a manual fallback if the user dismisses the installer dialog.
                MgUpdateChecker.installUpdate(activity, apkFile);
            }
